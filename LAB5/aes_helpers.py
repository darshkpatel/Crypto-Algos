...
def str_to_hex(st):
    return list(map(lambda x:int(x,16), st))
def str_to_hex_arr(stx):
    return [list(map(lambda x:int(x,16), st)) for st in stx]
def int_to_hex(st):
...
def substitute_sbox(word):
    ...
    new = []
    for byt in word:
        new.append(hex(Sbox[int(byt,16)])[2:])
    return new
def substitute_inv_sbox(word):
    #hex(Sbox[int("20",16)])
    new = []
    for byt in word:
        new.append(hex(inv_sbox[int(byt,16)])[2:])
    return new
